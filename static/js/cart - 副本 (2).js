...
(function () {
  ...
  $('.buy-btn').click(function (e) {
    ...
    if (shopping_cart.length <= 0 && $('#picmenu>.ui-draggable').length==0) {
      ...
      dom += '</table><div><br><p><b>备注：</b>'+ remark +'</p></div><div class="foot"><span class="total">共：' + get_total() + ' 元</span><button type="button" id="buy-go" class="btn">提交订单</button></div>';
      ...
      $('#buy-go').unbind('click').bind('click', function () {
        ...
        $.ajax({
          ...
          data: "list=" + encodeURIComponent(JSON.stringify(shopping_cart)) + "&picmenu=" + encodeURIComponent(JSON.stringify(picmenu_list)) +"&shop_name=" + shop_name + "&shop_id=" + shop_id + "&remark=" + remark,
          dataType: 'json',
          success: function (data) {
            if (data.result == "success") {
              //清空localstorage
              storage.removeItem(shop_id);
              $('#confirm-list').empty().html('<div style="text-align:center;"><p>订单提交成功</p><p>倒计时 <span class="timeout">6</span> 秒后 <a href="/today.php">跳转到今日订单</a></p></div>');
              ...
              setInterval(function () {
				...
				$('#confirm-list').empty().html('<div style="text-align:center;"><p>下单失败！</p><p>您已经在【'+shop_name+'】下过定单，并已经支付！</p><p>可转至 <a href="/today.php">今日订单</a> 查看</p></div>');
			}else if (data.result == "order_not_paid"){
				$('#confirm-list').empty().html('<div style="text-align:center;"><p>下单失败！</p><p>您已经在【'+shop_name+'】下过定单，但未支付！</p><p>请跳转至 <a href="/today.php">今日订单</a> 进行支付</p></div>');
			}
          },
          error: function () {
            alert('下单出错了');
